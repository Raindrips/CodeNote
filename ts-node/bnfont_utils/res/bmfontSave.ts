...
export function generateFntContent(fntData: FntData): string {
    ...
    const pagesStr = pages.map(page => `page id=${page.id} file="${page.file}"`).join('\n');
    const charsStr = chars.map(char => `char id=${char.id} x=${char.x} y=${char.y} width=${char.width} height=${char.height} xoffset=${char.xoffset} yoffset=${char.yoffset} xadvance=${char.xadvance} page=${char.page} chnl=${char.chnl}`).join('\n');
