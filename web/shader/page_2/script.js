...
async function main() {
    ...
    const foldProgressUniformLocation = gl.getUniformLocation(program, 'u_foldProgress');
    ...
    const pageImage = await loadImage('page1.jpg');

    const texture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, texture);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, pageImage);
    ...
    function drawScene() {
        ...
        foldProgress += (foldingForward ? 0.01 : -0.01);
        if (foldProgress >= 1.0) {
            foldingForward = false;
            foldProgress = 1.0;
        ...
        gl.uniform1f(foldProgressUniformLocation, foldProgress);
